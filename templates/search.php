...
// 获取最近10条搜索历史
$history = [];
$stmt = $pdo->prepare("SELECT keyword FROM search_history WHERE user_id = ? ORDER BY searched_at DESC LIMIT 10");
...
if ($searchTerm !== '') {
    ...
    $sql = "SELECT id, title, description, category, file_path
            FROM videos
...
<head>
    ...
    <style>
        ...
        .search-history { margin-bottom: 18px; }
        .search-history-title { color: #1976d2; font-weight: bold; margin-bottom: 6px; }
        .search-history-list { display: flex; flex-wrap: wrap; gap: 8px; }
        .search-history-item { background: #e3f2fd; color: #1976d2; border-radius: 6px; padding: 4px 12px; cursor: pointer; font-size: 15px; transition: background 0.2s; }
        .search-history-item:hover { background: #1976d2; color: #fff; }
    </style>
</head>
<body>
    <div class="search-container">
        ...
        <div class="search-history">
            ...
            <div class="search-history-list">
                <?php foreach ($history as $kw): ?>
                    <span class="search-history-item"><?php echo htmlspecialchars($kw); ?></span>
                <?php endforeach; ?>
            </div>
        </div>
        <?php endif; ?>
    ...
    <script>
    // 搜索历史点击
    document.querySelectorAll('.search-history-item').forEach(function(item){
